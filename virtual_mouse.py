...
#Tamaño del video que enfoca mi mano
width  = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) 
height  = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) 

#Tamaño de mi pantalla, escritorio
...
#declaro estado de pellizco en falso
pinch=False

# Definir una función para contar dedos
def countFingers(image, hand_landmarks, handNo=0):

    #Se tiene que volver global para usar en la función
	global pinch
	...
	if hand_landmarks:
		...
		indiceX=int((landmarks[8].x)* width)
		indiceY=int((landmarks[8].y)* height)

		pulgarX=int((landmarks[4].x)* width)
		pulgarY=int((landmarks[4].y)* height)

        #cv2.line(image,(indiceX,indiceY),(pulgarX,pulgarY),(255,0,0),2)

		# Dibujar un círculo en el centro de la línea entre la punta del dedo y la punta del pulgar


		center_x=int((indiceX+pulgarX)/2)
		center_y=int((indiceY+pulgarY)/2)

		cv2.circle(image,(center_x,center_y),2,(0,0,255),2)

		# Calcular la distancia entre la punta del dedo y la punta del pulgar
		distance=math.sqrt(((indiceX-pulgarX)**2)+((indiceY-pulgarY)**2))
